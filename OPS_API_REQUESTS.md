...
### #61-A `force_closed` 의미론 변경 — v2.15.14 도입 → v2.15.16 폐기 (FALSE 통일) — ⚠️ VIEW 측 뱃지 렌더링 영향

**우선순위**: 🟡 **MEDIUM** (FE 렌더링 영역 정합 확인 필요 — Sprint 33 FE-15 구현 전 영역 의미론 확정)
**Sprint**: AXIS-VIEW Sprint 33 FE-15 (force_closed 뱃지 렌더링) — 본 entry 영역 의미론 확정 후 진행 권고
**날짜**: 2026-05-15 (OPS v2.15.16 release 직후 등록)
**관련 FE**: AXIS-VIEW Sprint 33 FE-15 (뱃지 렌더링 구현 영역 영향)
**상태**: 📌 **INFO — OPS 측 v2.15.16 완료, VIEW FE 영역 의미론 정합 확인 필요**

---

#### 1. 변경 trail (OPS 측)

| OPS release | 정책 | 발동 경로 |
|-------------|------|-----------|
| ~v2.15.13 | auto-close 영역 force_closed=TRUE (default) | Sprint 41-B 레거시 루프 + Sprint 41-D trigger |
| v2.15.14 (5-15) | **분기**: `unfinished_workers_count > 0` 일 때 TRUE / 0 일 때 FALSE | task_service / checklist_service auto-close 5곳 |
| **v2.15.16 (5-15)** | **FALSE 통일** — auto-close 영역 자연 close 처리 / TRUE 영역 manager force-close API 전용 | task_service / checklist_service 5곳 + 레거시 루프 제거 |

#### 2. 사용자 분석 (v2.15.16 root cause)

- close trigger 자체가 정상 시점 (근무시간 내 발동)
- 미완료 worker 존재해도 → trigger 시간 = 정상 close 시간 (force_closed=FALSE)
- 조건 1 (attendance check_out) + 조건 2 (17:00 fallback) 영역 무의미 → 정책 폐기
- 강제종료 영역 manager force-close API 영역만 의미 있음

#### 3. VIEW FE 측 영향

| 영역 | v2.15.16 이후 동작 | 정합 확인 |
|------|-------------------|----------|
| **force_closed=TRUE 뱃지 표시** | **manager force-close API 영역만 표시** (auto-close 영역 모두 FALSE) | ✅ 라벨 "강제종료" 영역 의미 명확 (이전 영역 자동 close 영역 포함되어 혼란 있었음) |
| **auto-close task 영역** | force_closed=FALSE → 자연 close 영역 정상 완료 task 와 동일 표시 | ⚠️ close_reason prefix `AUTO_CLOSED_BY_*` 영역 영역 구분 가능 (별 표시 영역 권고 검토) |
| **closed_by_name 표시** | 옵션 B 채택 — auto-close 영역 last_completion_worker_id 보존 / manual 영역 관리자 worker_id | ✅ 정보 가치 보존 |
| **duration_source 표시** (있다면) | 신규 enum `PREV_DAY_CAP` 추가 (익일/주말 trigger duration cap) | ⚠️ VIEW 측 표시 영역 없으면 영향 0 / 있으면 렌더링 추가 권고 |

#### 4. close_reason audit trail (참조)

```
AUTO_CLOSED_BY_FIRST_FINAL_TRIGGER:{task_id}  → Sprint 41-D First Close trigger
AUTO_CLOSED_BY_SECOND_FINAL_TRIGGER:{task_id} → Sprint 41-D Second Close trigger
MANUAL_FORCE_CLOSE                            → manager force-close API 영역만
AUTO_CLOSED_LEGACY                            → v2.15.16 부터 신규 발생 0건 (Sprint 41-B 레거시 루프 제거)
```

#### 5. duration_source enum (v2.15.16 5종)

```
NORMAL_COMPLETION       → worker 정상 complete_work()
PREV_DAY_CAP            → 신규 (v2.15.16) — 익일/주말 trigger 영역 started.date() 17:00 cap
ATTENDANCE_OUT          → orphan worker attendance check_out
FALLBACK_TRIGGER_DATE_17→ 위 3건 없을 때 17:00 fallback
INVALID_WARNING         → duration_validator 비정상 검출
```

#### 6. VIEW FE 측 액션 권고

- **Sprint 33 FE-15** (force_closed 뱃지 렌더링) 영역 v2.15.16 의미론 영역 구현 진행:
  - `force_closed === true` → "🔒 강제종료 (관리자)" 라벨
  - `force_closed === false && close_reason?.startsWith('AUTO_CLOSED')` → "🤖 자동 close" 라벨 (선택)
  - `force_closed === false && close_reason === null` → 일반 완료 task (라벨 없음)
- **추정**: Sprint 33 FE-15 영역 진행 시 의미론 정합 검토 ~10분 추가

#### 7. 관련 OPS 영역

- OPS BACKLOG: `POST-REVIEW-AUDIT-TRAIL-CONSISTENCY-20260515` (deadline 2026-05-22)
- OPS AUDIT_TRAIL_GUIDE.md (L6+L19+L41~43) — closed_by 옵션 B 채택 + force_closed 의미론 명시
- OPS CHANGELOG.md [2.15.16] entry

#### 8. 운영 검증 정합

- v2.15.16 운영 검증 진행 중 (사용자 측 5-15)
- 결과 정합 시 VIEW Sprint 33 FE-15 영역 본 의미론 채택
