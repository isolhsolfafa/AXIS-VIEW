// src/hooks/useCompleteTaskBatch.ts
// Sprint 40 — Tank Module 일괄 종료 (Promise.allSettled fallback + partial-success)

import { useMutation, useQueryClient } from '@tanstack/react-query';
import { completeTask, completeTaskBatch, type BatchResponse } from '@/api/snStatus';

export function useCompleteTaskBatchMutation(_serialNumber: string) {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (taskDetailIds: number[]): Promise<BatchResponse> => {
      try {
        return await completeTaskBatch(taskDetailIds);
      } catch (err: unknown) {
        const status = (err as { response?: { status?: number } })?.response?.status;
        if (status === 404) {
          const settled = await Promise.allSettled(
            taskDetailIds.map(id => completeTask(id)),
          );
