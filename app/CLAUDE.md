...
│   │   ├── production/                # 생산관리
│   │   │   ├── ProductionPlanPage.tsx  # 생산일정
│   │   │   ├── SNStatusPage.tsx        # 생산현황 (S/N 카드뷰)
│   │   │   ├── ProductionPerformancePage.tsx # 생산실적
│   │   │   └── ShipmentHistoryPage.tsx # 출하이력
│   │   ├── partner/                   # 협력사 관리
│   │   │   ├── PartnerDashboardPage.tsx
│   │   │   ├── PartnerEvaluationPage.tsx
│   │   │   └── PartnerAllocationPage.tsx
│   │   ├── qr/                        # QR 관리 + ETL
│   │   │   ├── QrManagementPage.tsx
│   │   │   └── EtlChangeLogPage.tsx
│   │   ├── checklist/                 # 체크리스트 관리
│   │   │   └── ChecklistManagePage.tsx
│   │   ├── admin/                     # 관리자 전용
│   │   │   ├── PermissionsPage.tsx     # 권한 관리
│   │   │   └── InactiveWorkersPage.tsx # 비활성 사용자 관리
│   │   ├── analytics/                 # 사용자 분석
...
├── 협력사 관리       → 펼침 메뉴             [admin, manager]
│   ├── 대시보드      → /partner              (준비중)
│   ├── 평가지수      → /partner/evaluation   (준비중)
│   ├── 물량할당      → /partner/allocation   (준비중)
│   └── 근태 관리     → /partner/attendance
└── 생산관리          → 펼침 메뉴             [admin, manager, gst]
    ├── 생산일정      → /production/plan
    ├── 생산현황      → /production/status
    ├── 생산실적      → /production/performance (준비중)
    └── 출하이력      → /production/shipment    (준비중)

Management
├── QR 관리           → 펼침 메뉴             [admin, manager, gst]
│   ├── QR Registry   → /qr
│   └── 변경 이력     → /qr/changes
...
Analysis
├── 불량 분석         → /defect               (준비중) [admin, gst]
├── CT 분석           → /ct                   (준비중) [admin, gst]
...
/factory → FactoryDashboardPage [admin, manager, gst]
/partner → PartnerDashboardPage [admin, manager]
/partner/evaluation → PartnerEvaluationPage [admin, manager]
/partner/allocation → PartnerAllocationPage [admin, manager]
/partner/attendance → AttendancePage [admin, manager, gst]
/production/plan → ProductionPlanPage [admin, manager, gst]
/production/status → SNStatusPage [admin, manager, gst]
/production/performance → ProductionPerformancePage [admin, manager, gst]
/production/shipment → ShipmentHistoryPage [admin, manager, gst]
/qr → QrManagementPage [admin, manager, gst]
/qr/changes → EtlChangeLogPage [admin, manager, gst]
/checklist → ChecklistManagePage [admin, manager, gst]
/admin/permissions → PermissionsPage [admin, manager]
/admin/inactive → InactiveWorkersPage [admin]
/defect → DefectAnalysisPage [admin, gst]
/ct → CtAnalysisPage [admin, gst]
/analytics → AnalyticsDashboardPage [admin]
/* → 404 페이지
```
...
| v1.5.0 | 2026-03-12 | Phase 5-A | 협력사 관리 메뉴 개편 + 대시보드/평가지수/물량할당 + 근태 자사 필터 |
| v1.6.0 | 2026-03-13 | Sprint 27 | 권한 매트릭스 세분화 (GST 일반/협력사 manager 분리) |
| v1.7.0 | 2026-03-16 | Sprint 29 | 공장 API 실데이터 연동 + 대시보드/생산일정 리팩토링 |
| v1.8.0 | 2026-03-18 | Sprint 5 | QR DUAL Tank 뱃지 + 기구/모듈 필터 분리 + ETL 오늘 필터 |
| v1.9.0 | 2026-03-20 | Sprint 6~8 | 태스크 진행률 + 사용자 분석 대시보드 + 생산실적 API 연동 |
| v1.10.0 | 2026-03-23 | Sprint 13 | 생산실적 공정 그룹 탭 분리 + vitest 테스트 환경 |
| v1.13.0 | 2026-03-24 | Sprint 16 | S/N별 실적확인 UI + End 필터 + SNConfirmButton |
| v1.14.0 | 2026-03-25 | Sprint 18 | S/N 작업 현황 카드뷰 (생산현황 페이지 + 상세 패널) |
| v1.15.0 | 2026-03-26 | Sprint 20 | 체크리스트 관리 목업 + 생산현황 체크리스트 연동 |
| v1.15.1 | 2026-03-27 | Sprint 18-B | 상세뷰 UX 개선 (정렬 반전 + 체크리스트 토글 + task 이름 표시) |
| v1.15.2 | 2026-03-27 | Sprint 18-B+ | 생산현황 공정 탭 제거 + 협력사 권한 필터 |
