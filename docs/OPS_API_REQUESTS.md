...
**FE 현황**: AnnouncementPanel UI 구현 완료 (Mock 데이터 사용 중). `/api/notices` 연동으로 Mock → API 전환 필요.
...
**VIEW 작업**: Mock → API 전환만 필요. BE 수정 불필요.
...
**FE 현황**: `EtlChangeLogPage.tsx` Mock UI 완료 (`/qr/changes`). API 연동 시 Mock → API 전환.
