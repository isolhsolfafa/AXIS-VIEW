...
# Sprint 18 (APP): S/N 카드뷰 — 작업자 태깅 현황 조회 (2026-03-25)

> **목적**: 작업자가 APP에서 태깅한 S/N 목록을 카드뷰로 확인하고, 상세 클릭 시 공정별 작업자·시간 이력을 조회
> **범위**: APP(React) FE only — OPS BE 추가 개발 없음 (기존 API 활용)
> **원칙**: APP = 태깅 + 최소 확인 / VIEW 대시보드 = 분석

## 배경

현재 APP에서 QR 태깅은 가능하지만, 태깅 결과를 S/N 단위로 확인하는 화면이 없음.
작업자가 "내가 태깅한 S/N이 어디까지 진행되었는지", "각 공정에 누가 작업했는지"를 확인하려면 VIEW 대시보드에 접속해야 하는 상황.

APP 내에서 **최소한의 정보**만 카드뷰로 제공하여 현장 확인 용도로 사용.

## 기존 API 활용 (OPS BE 변경 없음)

| 용도 | API | 응답 데이터 |
|------|-----|------------|
| S/N 리스트 + progress | `GET /performance` | `sn_confirms[]` — 공정별 confirm 현황 |
| S/N 상세 (작업자별 시간) | `GET /tasks/<serial_number>?all=true` | `workers[]` — worker_name, started_at, completed_at, duration_minutes, status |
| 실시간 push | WebSocket `/ws` | `emit_task_completed()` — 태깅 발생 시 broadcast |
...
로그인 작업자의 **소속 협력사(company) 자동 필터링** — 근태관리 페이지와 동일 방식.
JWT에서 worker_id 추출 → workers 테이블에서 company 조회 → 해당 협력사 S/N만 노출.

---

## Task 1: S/N 카드 리스트 화면 (Screen 1)
...
┌─────────────────────────────────┐
│  🔍 S/N 검색                     │
│  [MECH] [ELEC] [TM] [전체]      │  ← 공정 필터 탭
├─────────────────────────────────┤
│  ┌─────────────────────────────┐│
│  │ GBWS-6408   GAIA-I DUAL    ││
│  │ ■■■■■□□  4/7 공정 완료      ││
│  │ MECH ✓  ELEC ✓  TM ●  PI ○ ││
│  │ 마지막: 김태영 10:32         ││
│  └─────────────────────────────┘│
│  ┌─────────────────────────────┐│
│  │ GBWS-6409   GAIA-I DUAL    ││
│  │ ■■□□□□□  2/7 공정 완료      ││
│  │ MECH ✓  ELEC ●  TM ○  PI ○ ││
│  │ 마지막: 박준호 09:15         ││
│  └─────────────────────────────┘│
...
- `GET /performance` → 응답의 `orders[].sns[]` + `orders[].sn_confirms[]`
- 카드 1장 = S/N 1개
- progress 계산: `sn_confirms`에서 해당 S/N의 confirm된 공정 수 / 전체 공정 수

### 카드 표시 항목

| 항목 | 소스 |
|------|------|
| S/N | `sn.serial_number` |
| 모델명 | `order.model_name` |
| progress bar | `sn_confirms` 기반 — confirm된 공정 / 전체 공정 |
| 공정 아이콘 | MECH/ELEC/TM/PI/QI/SI — ✓완료 / ●진행중 / ○대기 |
| 마지막 태깅 | `sn_confirms` 중 가장 최근 confirmed_at + worker_name |

### 공정 필터 탭

| 탭 | 필터 기준 |
|----|----------|
| MECH | MECH 공정이 있는 S/N만 |
| ELEC | ELEC 공정이 있는 S/N만 |
| TM | TM 공정이 있는 S/N만 |
| 전체 | 필터 없이 전체 |

### 정렬

기본: 진행중(●) > 대기(○) > 완료(✓) 순서. 같은 상태 내에서는 최근 태깅 시간 내림차순.

---

## Task 2: S/N 상세 화면 (Screen 2) — 카드 클릭
...
- `GET /tasks/<serial_number>?all=true`
- 각 task의 `workers[]` 배열에서 작업자 정보 추출

### 공정별 카드 표시 항목

| 항목 | 소스 |
|------|------|
| 공정명 | task.task_category (MECH/ELEC/TM/PI/QI/SI) |
| 상태 | ✅ completed / 🔵 in_progress / ○ 대기 |
| 작업자명 | `workers[].worker_name` |
| 시작 시간 | `workers[].started_at` |
| 완료 시간 | `workers[].completed_at` |
| 소요 시간 | `workers[].duration_minutes` → 포맷: "1h 32m" |

### 공정 순서 (고정)

MECH → ELEC → TM → PI → QI → SI

task_category 기준으로 그룹핑 후 위 순서대로 정렬.
동일 공정 내 다중 작업자(교대/인수인계)는 workers[] 배열 순서대로 표시.

---

## Task 3: WebSocket 연동 — 실시간 카드 갱신

### 현재 구조 (변경 없음)

OPS BE의 `emit_task_completed()`가 태깅 완료 시 전체 broadcast:
...
1. WebSocket `task_completed` 이벤트 수신
2. 해당 `serial_number`가 현재 리스트에 있으면 → 해당 카드 progress 갱신
3. 상세 화면에서 해당 S/N을 보고 있으면 → 공정 카드 상태 업데이트
4. 갱신 방식: 이벤트 수신 시 해당 S/N의 데이터만 re-fetch (전체 리스트 재조회 아님)

### VIEW 대시보드

현재 실시간 화면 새로고침 불필요. 기존 페이지 진입 시 자동 fetch + 수동 새로고침으로 충분.
...
| 파일 | 변경 | 비고 |
|------|------|------|
| `SNCardListScreen` (신규) | S/N 카드 리스트 화면 | Screen 1 |
| `SNDetailScreen` (신규) | S/N 상세 화면 | Screen 2 |
| `SNCard` (신규) | 카드 컴포넌트 | progress bar + 공정 아이콘 |
| `ProcessStepCard` (신규) | 공정별 상세 카드 | 작업자 + 시간 |
| Navigation | 라우팅 추가 | 카드 리스트 → 상세 이동 |

**OPS BE: 변경 없음** — 기존 `GET /performance`, `GET /tasks/<sn>`, WebSocket `/ws` 활용
...
**APP FE**:
- [ ] S/N 카드 리스트 화면 구현
- [ ] 협력사 자동 필터링 (JWT company 기반)
- [ ] 공정 필터 탭 (MECH/ELEC/TM/전체)
- [ ] 카드 progress bar (sn_confirms 기반)
- [ ] S/N 상세 화면 구현
- [ ] 공정별 작업자·시간 표시 (workers[] 기반)
- [ ] 공정 순서 고정 (MECH→ELEC→TM→PI→QI→SI)
- [ ] WebSocket task_completed 이벤트 → 카드 갱신
- [ ] 검색 기능 (S/N, 모델명)
- [ ] 정렬 (진행중 > 대기 > 완료)

**검증 (배포 후)**:
- [ ] 🔴 로그인 시 소속 협력사 S/N만 표시
- [ ] 🔴 카드 클릭 → 상세 화면 전환 정상
- [ ] 🔴 작업자별 시작/완료 시간 표시 정확
- [ ] 🔴 다른 작업자 태깅 시 카드 실시간 갱신
- [ ] 🔴 공정 필터 탭 전환 정상
...
- **OPS BE 수정 금지** — 기존 API만 활용. 새 엔드포인트 추가 불필요
- APP은 **최소 정보만 표시** — 상세 분석은 VIEW 대시보드
- 공정 순서 **MECH → ELEC → TM → PI → QI → SI 고정** — model_config에 따른 동적 순서는 추후 Sprint
- progress 계산은 `sn_confirms` 기준 — FE에서 별도 재계산 금지
- 카드 갱신은 **해당 S/N만 re-fetch** — 전체 리스트 재조회 금지 (성능)
- 협력사 필터는 **서버 사이드** (JWT 기반) — 클라이언트에서 전체 데이터 받아서 필터링 금지
- WebSocket 이벤트 수신 실패 시 **silent fail** — 에러 팝업 없이 수동 새로고침으로 보완
- ⚠️ `.env` 절대 커밋 금지
- 완료 시 DESIGN_FIX_SPRINT.md 체크리스트 업데이트
