...
> **범위**: VIEW(React 19 + TanStack Query v5) FE — OPS BE: `last_worker` 필드 추가 (Sprint 38, 선행 불필요)
...
> **방향 B 채택 (OPS Sprint 38)**: 카드 리스트에서 최근 태깅 작업자를 표시하기 위해 `GET /api/app/product/progress` 응답에 `last_worker`, `last_activity_at` summary 필드를 OPS BE에서 추가. N+1 호출 방지.
> Sprint 38 BE 배포 전까지는 카드에 작업자 정보 없이 progress만 표시, 배포 후 FE에서 필드 연결만 추가.
...
> `last_worker` / `last_activity_at`는 Sprint 38 이후 추가될 필드. Sprint 38 전에는 `null`.
...
- 최근 태깅: `product.last_worker`, `product.last_activity_at` (Sprint 38 이후 연결)
...
| 최근 작업자 | `product.last_worker` | Sprint 38 전: null → 숨김 |
| 마지막 태깅 시간 | `product.last_activity_at` | Sprint 38 전: null → 숨김 |
...
같은 상태 내에서는 `last_activity_at` 내림차순 (Sprint 38 전에는 `ship_plan_date` ASC 대체).
...
**OPS BE**: Sprint 38에서 `last_worker`, `last_activity_at` 추가 (선행 불필요 — Sprint 18은 null 허용)
...
- **OPS BE Sprint 38 선행 불필요** — `GET /api/app/product/progress` 기존 API 활용. `last_worker` 필드만 Sprint 38 이후 연결 (null이면 숨김 처리)
