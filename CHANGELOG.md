...
**Sprint 46 hotfix — SI 공정 토글 정상화**

🔧 수정
 - 생산현황에서 SI 토글을 켜고 꺼도 목록이 안 바뀌던 문제 수정 (Twin파파 catch)
 - 원인: SI 토글이 "미완료"만 보고 "마무리공정 시작 여부"를 안 봐서 — 출하 전 거의 모든 제품이 노출돼 토글이 무의미했음
 - 수정 후: SI 토글 = 마무리공정을 시작했고 + 아직 출하 완료 안 된 제품 (출하 대기 목록)

✅ 검증
 - vitest 15/15 PASS, 빌드 GREEN
 - PI/QI 토글과 동일하게 "작업 시작 이력(태깅)" 기준으로 통일

---

## v1.46.0 — 2026-05-18

**생산현황 PI/QI/SI 공정 토글 필터 (Sprint 46)**

🔎 공정 토글 필터
 - 생산현황 설정에 PI / QI / SI 토글 추가 (GST·admin)
 - 보고 싶은 공정만 켜서 해당 S/N 만 추림 — 다중 선택 가능, 전부 끄면 전체보기
 - PI/QI: 진행 중이거나 오늘 완료된 S/N (완료 후 다음날 자동 제외)
 - SI: 마무리 미완료 S/N (= 출하 대기 목록)

🔄 이전 버전 대비
 - v1.45.0 의 "GST 작업자 자기 공정 고정" 자동 스코프는 폐기 — 권한 제한이 아닌 편의 필터로 전환
 - 테스트 S/N 전용 보기(v1.45.0)는 유지

✅ 검증
 - 빌드 GREEN, vitest 89/89 PASS
...
📖 메뉴얼 바로가기
 - 상단바에 메뉴얼 버튼 추가 (공지사항·설정 사이, 책 아이콘)
 - 클릭 시 사용 설명서(axis-manual) 새 탭으로 열림
 - 동작 없던 기존 검색 버튼은 제거
...
**생산현황 view/filter 정리 (Sprint 45)**

🔧 테스트 S/N 토글 — 의미 변경
 - 기존: ON 시 운영+테스트 혼재 표시
 - 변경: **ON = 테스트 S/N 전용 / OFF = 운영 S/N 전용** (혼재 제거)
 - 설정 라벨 "테스트 S/N 표시" → "테스트 S/N 전용 보기"

👤 GST 자체공정(PI/QI/SI) 작업자별 view
 - GST 일반 작업자는 자기 공정만 표시 — PI 작업자 → PI, QI 작업자 → QI, SI 작업자 → SI (협력사 view 와 동일 방식)
 - 매니저 권한(`is_manager`)만 전체 6공정 view
 - 진행률·S/N 목록 모두 담당 공정으로 스코프

✅ 검증
 - BE 변경 0 (계정 권한 정보 기존 필드 활용)
 - Codex 교차검증 반영, 빌드 GREEN, vitest 84/84 PASS

---

## v1.44.2 — 2026-05-18

**FIX-FORCE-CLOSED-REACTIVATION — 강제종료 task 재활성화 버튼 표시 일관성**

🔧 수정
 - 생산현황 상세화면에서 강제종료한 작업의 재활성화 버튼이 일관되게 표시되도록 정정
 - 이전: 작업자가 시작했던 작업을 강제종료하면 재활성화 버튼이 보이는데, 시작도 안 한 작업을 강제종료하면 버튼이 안 보였음
 - ProcessStepCard 재활성화 버튼 조건을 `완료 또는 강제종료` 로 확장

✅ 검증
 - tsc -b 에러 0, vite build GREEN
 - 동반 OPS v2.15.20 — 재활성화 시 강제종료 표시(force_closed 등 4컬럼)도 함께 해제
 - Codex 라운드 1 합의 (노출 범위 정확 — 진행중/정상 미시작 작업엔 버튼 미노출 유지)

---

## v1.44.1 — 2026-05-18

**Sprint 44 후속 — 공장 대시보드 차트 정리 (자가 검토 반영)**

🔧 수정
 - 월간 뷰에서 좌측 막대 차트가 도넛 카드 높이에 맞춰 늘어나도록 정리
 - 막대 위 생산 수량 숫자가 막대를 따라가도록 정정 (이전엔 맨 위 고정)
 - "전체 새로고침" 시 고객사 도넛도 함께 갱신되도록 누락 보완

📋 후속 (BE 요청)
 - 월간 생산량 KPI 가 테스트 데이터(TEST CUSTOMER)를 제외하도록 BE 요청 등록 (도넛 숫자와 일치 — OPS #69)

✅ 검증
 - 빌드 GREEN, vitest 72/72 PASS

---

## v1.44.0 — 2026-05-18

**공장 대시보드 — 월생산 고객사 비율 도넛 (Sprint 44)**

✨ 신규
 - 공장 대시보드 우측 패널이 주간/월간 토글에 연동
 - 주간 뷰 → 공정별 완료율 (기존)
 - 월간 뷰 → 월생산 고객사 비율 도넛 (상위 5개 고객사 + 기타, 중앙에 총 생산 대수)
 - 30초마다 대시보드 전체와 함께 자동 전환

🔧 구성
 - 월간 생산을 고객사별로 집계해 표시 (BE 신규 집계 연동)
 - 날짜 기준은 설정 패널의 월간 기준 토글을 따름
 - 생산현황 테이블/차트는 영향 없음 (기존 기구 착수일 기준 유지)

✅ 검증
 - Codex 교차검증 반영, 빌드 GREEN, vitest 72/72 PASS

---

## v1.43.14 — 2026-05-18

**QR 관리 CSV 다운로드 — 협력사 컬럼 추가 (Twin파파 요청)**

➕ 추가
 - QR 관리 페이지 CSV 다운로드에 기구협력사 / 전장협력사 컬럼 추가

✅ 검증
 - BE 변경 0 (QR 목록 API 가 협력사 필드 이미 응답), 빌드 GREEN

---

## v1.43.13 — 2026-05-16

**협력사 관리 체크리스트 성적서 검색 정합 (Twin파파 catch)**

🐛 증상
 - 체크리스트 성적서 검색에서 O/N 은 숫자만 입력해도 나오는데, S/N 은 앞에 영문(prefix)까지 쳐야 나옴

🔧 수정
 - 검색어를 O/N·S/N 양쪽으로 동시에 조회하도록 정정
 - 이제 숫자 일부만 입력해도 O/N·S/N 양쪽에서 검색됨 (S/N 영문 prefix 불필요)

✅ 검증
 - BE 변경 0 (엔드포인트가 OR 조건·부분 매칭 이미 지원)
 - 빌드 GREEN, vitest 64/64 PASS

---

## v1.43.12 — 2026-05-15

**생산현황 상세화면 Tank Module 일괄 시작/종료 — DUAL 모델 fix (Twin파파 catch)**

🐛 증상
 - DUAL 모델(L/R 2개) Tank Module 을 일괄 시작/종료할 때 1개만 처리됨

🔧 수정
 - DUAL 모델 L/R 2개 task 를 모두 일괄 처리하도록 정정
 - L 진행 중·R 미시작 등 부분 상태에서 시작/종료 버튼이 알맞게 표시
 - 강제종료된 task 는 일괄 대상에서 제외

✅ 검증
 - BE 변경 0
 - Codex 교차검증 M 3건 반영
 - 빌드 GREEN, vitest 62/62 PASS

> v1.43.11 + v1.43.12 는 한 번에 배포됨 (APP_VERSION = v1.43.12)

---

## v1.43.11 — 2026-05-15

**MECH 체크리스트 1차+2차 검수 합산 + 상세화면 폰트 확대 (Twin파파 catch)**

🐛 증상
 - 생산현황 상세화면 MECH 체크리스트가 11개(1차 검수)만 카운트 — 2차 검수가 빠짐

🔍 원인
 - MECH 체크리스트는 1차 검수(작업자) / 2차 검수(관리자 보강) 2단계 구조
 - 상세화면이 1차 검수만 조회 → 2차 검수 항목 미포함
 - MECH 완료 판정은 1차+2차 모두 기준 → 표시와 완료 기준이 어긋남

🔧 수정
 - MECH 체크리스트를 1차+2차 합산해서 1개 진행 바로 표시 (완료 판정과 일치)
 - 100% = 1차 검수·2차 검수 모두 완료
 - 펼침 시 미완료 항목에 "1차 · / 2차 ·" 구분 표기

🔎 상세화면 폰트 확대
 - 생산현황 S/N 상세화면 글자 크기 한 단계씩 키움 (너무 작다는 피드백 반영)
...
**UX 일관성 — O/N 그룹 토글 단일 패턴 통일**

### 배경
- v1.36.0 Sprint 37 배포 후 운영 피드백: 다대(2대+) 는 접혀 있고 단대(1대) 는 펼쳐 있어 시각적 일관성 부족
- "왜 어떤 건 안 펼쳐지지?" 의문 발생 가능

### 변경
- `multi`(products.length ≥ 2) 분기 제거 → `hasHeader`(salesOrder 존재 여부) 분기로 통일
- O/N 번호가 있는 모든 그룹(단대·다대 무관)이 동일하게 **클릭으로 펼침/접힘**
- 1대짜리도 chevron + 카드형 헤더 + hover 효과 동일 적용
- O/N 없음 그룹(`_no_on_{sn}`)은 헤더 자체가 없어 기존대로 항상 노출

### 영향
- 첫 진입 시 화면 길이 더 압축 (1대 그룹도 접힘 상태로 시작)
- O/N 단위 시각적 패턴 일관성 ✅
- 1대 그룹 카드 보려면 클릭 1단계 추가 (헤더에 진행률·라인·고객 정보 다 노출되므로 펼치지 않아도 의사결정 가능)
- BE API 변경 없음
- SNStatusPage.tsx 424 → 421 LOC (분기 제거 -3줄)
...
**HOTFIX — 공장 대시보드 "출하예정" 컬럼 매핑 정정**

### 증상
- 공장 대시보드 > 생산 현황 상세 테이블의 **출하예정 컬럼이 대부분 비어있음 (—)**

### 원인
- 컬럼 라벨은 "출하예정"인데 실제 소스 필드는 `finishing_plan_end`(마무리계획종료일)을 참조 중이었음
- v1.7.0(2026-03-16) 최초 도입 당시부터 라벨과 필드 의미가 불일치
- 실운영에서 `finishing_plan_end` NULL 비율이 증가하며 노출됨

### 수정
- `FactoryDashboardPage.tsx:446` 단일 라인 매핑 교정
  - before: `{r.finishing_plan_end?.slice(5) || '—'}`
  - after: `{r.ship_plan_date?.slice(5) || '—'}`
- 라벨 "출하예정" ↔ 필드 `ship_plan_date` 정합성 복원

### 영향
- BE API 변경 없음 (응답 필드는 기존부터 제공 중)
- 회귀 위험 없음 (라벨과 데이터 의미 일치)
