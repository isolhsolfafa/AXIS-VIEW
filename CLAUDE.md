...
> 최종 갱신: 2026-05-18 | 버전: v1.46.0 (Sprint 46 — 생산현황 PI/QI/SI 공정 토글 필터)
...
| Sprint 46 (v1.46.0) | 생산현황 PI/QI/SI 공정 토글 편의 필터 (FEAT-SNSTATUS-PROCESS-TOGGLE-FILTER, Twin파파 재결정). Sprint 45 의 GST `role` 자동 스코프(권한 제한)는 폐기 — GST 전공정 봐도 무방 → **토글 편의 필터**로 전환. **(1) Sprint 45 기능 B 롤백** — `companyScopedProgress`/`SNCard`/`SNDetailPanel`/`SNStatusPage` `git checkout a8e5813^` 복원 (Sprint 41 상태). 기능 A(테스트 토글 의미 변경 `matchesTestFilter`)는 유지. **(2) PI/QI/SI 토글 신규** — `SNStatusSettingsPanel` 토글 3개(GST/admin, `ToggleRow` 추출) + `DashboardSettings.processFilters` + `processToggleFilter.ts` `isVisibleForProcessToggle` 순수함수. 표시 조건: PI/QI=`started`(work_start_log) AND (미완료 OR `completed_today`), SI=`done<total`. 다중 OR 합집합, 전부 OFF=전체보기. BE `progress_service` 가 `categories` 에 `started`/`completed_at`/`completed_today` 추가(OPS 완료). Codex 1라운드 M1(`work_start_log`)/A6 → 설계서 영역 9 반영. 신규 2(`processToggleFilter`+test 13 TC) + 수정 4 + 롤백 4. 빌드 GREEN + vitest 89/89, 2026-05-18 | ✅ 완료 |
...
| v1.43.12 | 생산현황 상세화면 Tank Module 일괄 시작/종료 — DUAL 모델 L/R 2개 중 1개만 처리되던 버그 fix (Twin파파 catch). 원인: `SNDetailPanel.tsx` 가 현재 S/N 의 TANK_MODULE task 를 `catTasks.find(isTankModule)` 로 첫 1개만 잡음 → DUAL 모델은 L/R 2개 task 인데 1개만 batch 포함. fix: `.find()` → `.filter()` 배열화 (`tankTask` → `tankTasks`), `parallelDialog` state `taskId` → `taskIds`, `handleTankStart/Complete` 배열 수신 + single/batch 분기, `TankModuleActions` props `task` → `tasks`. Codex 라운드 1 M=3 반영 (force_closed task 제외 / L·R 부분 상태 시작·종료 버튼 동시 노출 / ParallelConfirmDialog 문구 정정). BE 변경 0. 2 파일 (`SNDetailPanel.tsx` + `ParallelConfirmDialog.tsx`). tsc/build GREEN + vitest 62/62. **v1.43.11 (MECH 체크리스트 합산 + 폰트) 와 동반 배포** (APP_VERSION = v1.43.12), 2026-05-15 | ✅ 완료 |
| v1.43.11 | MECH 체크리스트 1차+2차 검수 합산 + 상세화면 폰트 확대 (Twin파파 catch). v1.43.10 으로 MECH 체크리스트가 상세화면에 뜨기 시작했으나 1차 검수(phase 1) 11개만 카운트 — 2차 검수(phase 2) 미포함. 원인: `getChecklistStatus()` 가 BE status/detail 엔드포인트를 phase 파라미터 없이 호출 → BE 기본값 phase=1. MECH 체크리스트는 BE 에서 1차 검수(작업자)/2차 검수(관리자 보강) 2 phase 분리, 완료 판정 `check_mech_completion_all()` = phase1 AND phase2. fix: `getMechChecklistCombined()` 신규 — MECH 는 status/detail 각각 phase 1·2 분리 호출(4건) → summary 합산 + items 병합 (그룹 접두어 `1차 ·`/`2차 ·` + `MECH_PHASE2_KEY_OFFSET` 으로 React key 충돌 회피). `flattenChecklistDetail()` 헬퍼 추출 (ELEC/TM 단일 호출 경로 공용). + 상세화면 폰트 +1px (`SNDetailPanel`/`ProcessStepCard` 9~15px→10~16px). 4 파일 (`api/checklist.ts` + `checklist.test.ts` 10 TC + `SNDetailPanel` + `ProcessStepCard`). BE 변경 0. Codex 확인 라운드 GO (M 0건 / A-1 `ui_key` 분리 → BACKLOG `REFACTOR-CHECKLIST-UI-KEY`). 빌드 GREEN + vitest 62/62 PASS, 2026-05-15 | ✅ 완료 |
| HOTFIX v1.43.10 | 생산현황 상세화면 MECH 체크리스트 미표시 fix (Twin파파 catch). 원인: `api/checklist.ts` `getChecklistStatus()` 의 `CAT_MAP` 에 MECH 누락 — MECH 체크리스트 BE 엔드포인트(OPS Sprint 63-BE) 생성 전 작성된 코드라 MECH BE 추가 후 매핑 갱신 누락 → MECH 항상 `EMPTY_CHECKLIST` early-return → 상세화면 체크리스트 섹션 미렌더. `SNDetailPanel` 구조(`CHECKLIST_CATEGORIES`)는 이미 MECH 포함. fix: `CAT_MAP` 에 `MECH: 'mech'` 추가. **Codex M-1 동반 fix**: 상세 파서 transform 이 `id/status/worker_name` 기대했으나 BE(TM/ELEC/MECH 공통 `_get_checklist_by_category`)는 `master_id/check_result/checked_by_name` 반환 → `record` 항상 null + `master_id` 가 배열 index 대체되던 잠재 버그 (TM/ELEC 도 동일 영향) → BE 실제 필드명 우선 + legacy fallback 정정. 2 파일 (`api/checklist.ts` + `checklist.test.ts` 신규 7 TC). BE 변경 0. Codex 1차 NO-GO(M-1) → fix → 2차 GO. 빌드 GREEN + vitest 59/59 PASS, 2026-05-15 | ✅ 완료 |
...
| HOTFIX v1.43.5 | Sprint 40 일괄 토스트 BE 응답 schema 호환 — HOTFIX-TASKS-BY-ORDER-SCHEMA-20260511 (Twin파파 catch). 같은 O/N 다대 환경 (TEST-1111 + TEST-1112~1116) Tank Module ▶ 시작 시 일괄 모달 미발현. **원인**: BE Sprint 64-BE `/api/app/tasks/by-order/{ON}` 응답이 `{tasks, total}` 객체 wrap 인데 FE 는 `SNTaskDetail[]` 배열 직접 가정 → `Array.isArray=false` → 빈 배열 반환 → orderTasks=[] → getOtherSNsTankStartable=0 → 일괄 모달 안 뜸. **옵션 B 채택** (VIEW + OPS 동시 fix, 배포 순서 무관): VIEW `getTasksByOrder` 두 형식 호환 처리 (배열 + 객체) + OPS v2.13.1 응답 정합 별 repo 작업. 1 파일 fix (`api/snStatus.ts` +6 LoC) + 신규 test (`api/snStatus.test.ts` 73 LoC, 3 TC). **Codex 사후 검토 누락 영역**: Sprint 40 1·2·3·4·5차 검증 모두 다른 task endpoint 응답 형식 일관성 영역 검증 누락 (POST-REVIEW catch). Severity S2 (Sprint 40 핵심 기능 부분 장애). 빌드 GREEN + vitest 50/50 PASS (47 + 신규 3), 2026-05-11 | ✅ FE 완료 (OPS v2.13.1 별 repo 대기 — 배포 순서 무관) |
| HOTFIX v1.43.4 | Codex 사후 검토 (Task #20) M-01 + M-02 fix — HOTFIX-CHECKLIST-EDIT-DIRTY-GUARD-20260511. **M-01 (Late Hydrate Overwrite)**: `ChecklistEditModal` 에 `selectDirty` flag 도입 → 사용자가 자재코드 input 한 번이라도 건드리면 late hydrate skip (allMaterials 늦게 도착해도 입력값 덮어쓰기 차단). **M-02 (옵션 C invariant 미강제)**: SELECT 항목 + 매핑 0개 + 다른 필드만 수정 저장 차단 guard 추가 (`!hasPendingSelectChange` 분기에서 `existingCount === 0` 체크 → "SELECT 항목은 최소 1자재 매핑이 필요합니다" toast). 1 파일 fix (ChecklistEditModal.tsx +21/-3) + 테스트 +60 LoC (M-02 invariant TC 2건). Codex Advisory A-01 (Promise.all 부분 성공 롤백) + A-02 (직접 입력 vs 보조 모달 비활성 자재 정책 불일치) BACKLOG 이관 권장. Severity S3 (잠재 이슈 사전 차단, 운영 보고 0건). 빌드 GREEN + vitest 47/47 PASS, 2026-05-11 | ✅ 완료 |
